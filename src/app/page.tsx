...
export default function HomePage() {
  ...
  return (
    <motion.div
      className="min-h-screen pb-[181px] relative"
      animate={{ background: theme.bgGradient }}
      transition={{ duration: 0.8 }}
    >
      {/* Search bar - top 46px */}
      <div className="pt-[46px] px-[6px] mb-0">
        <div className="flex items-center h-[51px]">
          <div className="relative flex-1 h-[36px] flex items-center">
            ...
            <motion.div
              ...
              transition={{ duration: 0.8 }}
            >
              <span className="text-[17px] tracking-[-0.41px] leading-[22px]" style={{ color: 'rgba(60,60,67,0.6)', fontFamily: "'PingFang SC', sans-serif" }}>搜索</span>
            </motion.div>
          </div>
          <img src="/ui/avatar.png" alt="头像" className="w-[28px] h-[28px] rounded-full ml-[6px] flex-shrink-0" />
        </div>
      </div>

      {/* Record mood header bar - 68px tall, glass green bar */}
      <motion.div
        className="mx-[14px] rounded-t-[20px] h-[68px] flex items-center justify-between px-[14px] relative"
      ...
      >
        ...
        <div className="flex items-center gap-[4px]">
          <AnimatePresence mode="wait">
            <motion.div key={currentMood} className="flex items-center gap-[3px]" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }}>
      ...
      {/* Coordinate area - 358px tall, glass effect */}
      <motion.div
        className="mx-[14px] rounded-b-[20px] h-[358px] relative overflow-hidden"
        animate={{
        ...
        style={{
          ...
          border: `1px solid ${theme.textPrimary}38`,
          borderTop: 'none',
      ...
      {/* Section titles - 推荐歌单 / 个人歌单 */}
      <div className="flex items-baseline px-[19px] mt-[12px]">
        <button
          onClick={() => setListMode('recommend')}
          className="text-[20px] font-semibold leading-normal transition-colors"
          style={{ color: listMode === 'recommend' ? theme.textPrimary : `${theme.textPrimary}4D`, fontFamily: "'PingFang SC', sans-serif" }}
        >
          推荐歌单
        </button>
        <button
          onClick={() => setListMode('personal')}
          className="text-[20px] font-semibold leading-normal ml-[20px] transition-colors"
          style={{ color: listMode === 'personal' ? theme.textPrimary : `${theme.textPrimary}4D`, fontFamily: "'PingFang SC', sans-serif" }}
        >
          个人歌单
        </button>
      ...
      {/* AI message card - glass note */}
      <motion.div
        className="mx-[22px] mt-[8px] relative"
        animate={{ backgroundColor: `${theme.primaryLight}8C`, borderColor: 'rgba(136,120,73,0.18)' }}
        transition={{ duration: 0.8 }}
        style={{ border: '1px solid rgba(136,120,73,0.18)', height: 119 }}
      >
        <div className="px-[11px] pt-[16px]">
          <AnimatePresence mode="wait">
            <motion.p key={currentMood} className="text-[14px] font-medium leading-[1.5]" style={{ color: '#58480E', fontFamily: "'Noto Sans SC', sans-serif", width: 181 }}
        ...
        <button className="absolute top-[0px] right-[0px]">
          <img src="/icons/edit-note.svg" alt="" className="w-[32px] h-[32px] opacity-50" />
        </button>
      </motion.div>

      {/* Song list - rounded container with glass bg */}
      <motion.div
        className="mx-[20px] mt-[8px] rounded-[20px] overflow-hidden"
        ...
        <AnimatePresence mode="wait">
          <motion.div key={`${currentMood}-${listMode}`} className="py-[7px]"
            ...
            {songs.map((song, index) => (
              <div key={song.id}>
                <motion.div
                  className="flex items-center px-[8px] py-[0px] active:opacity-80"
                  whileTap={{ scale: 0.98 }}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 ml-[23px]">
                    ...
                    {listMode === 'personal' && <p className="text-[14px] leading-[1.5]" style={{ color: `${theme.textPrimary}80`, fontFamily: "'Noto Sans SC', sans-serif" }}>{song.date}</p>}
                  </div>
